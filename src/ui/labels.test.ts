...
describe("describePower", () => {
  it("incrusta los alimentos como marcadores de ícono, sin emoji", () => {
    ...
    expect(describePower(power)).toBe("Obtén 2 {seed} del comedero");
  });
  ...
  it("con alternativa lleva dos marcadores", () => {
    ...
    expect(describePower(power)).toBe("Obtén 1 {insect} o {seed}");
  });
  ...
  it("describePowerText pasa los marcadores a palabras", () => {
    ...
    expect(describePowerText(power)).toBe("Obtén TODOS los pez que haya en el comedero");
  });
  ...
  it("ningún texto de poder contiene emoji", () => {
    ...
    expect(describePower(power)).toBe("Cambia 1 {seed} por 2 {fruit}");
